...
The $\hat{i}$ and $\hat{j}$ can be interpreted as a 2x2 matrix describing a linear transformation. Then you you a given a vector as in this case $\begin{bmatrix} -1x \\ 2y \end{bmatrix}$, and then you can use the above calculation to find where it lands. For some other vector we recalculate where the vector lands. 
